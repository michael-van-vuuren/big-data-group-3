...
// Registers new user, then logs them in using newly made account
export async function POST(req: NextRequest) {
  try {
    ...
    const javaBackendUrl = `${JAVA_API_BASE_URL}/auth/register`;
    ...
    if (!response.ok) {
      ...
      console.error('Java Backend Register Error:', errorData);
    ...
    if (!token) {
       console.error('No token received from Java backend after registration');
       return NextResponse.json(
